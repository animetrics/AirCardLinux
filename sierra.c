			...
			const struct usb_device_id *id)
{
	...
	if (serial->interface->num_altsetting == 2) {
		pr_debug( "Selecting alt setting for interface %d\n", /*dev_dbg(&udev->dev, "Selecting alt setting for interface %d\n",*/
			ifnum);
	...
	if (is_blacklisted(ifnum,
		...
		pr_debug( /*dev_dbg(&serial->dev->dev,*/
...
static void sierra_release_urb(struct urb *urb)
{
	...
	if (urb) {
		port =  urb->context;
		pr_debug( "%s: %p\n", __func__, urb); /*dev_dbg(&port->dev, "%s: %p\n", __func__, urb);*/
		usb_free_urb(urb);
	}
}
...
static void sierra_outdat_callback(struct urb *urb)
{
	...
	pr_debug( "%s - port %d\n", __func__, port->number); /*dev_dbg(&port->dev, "%s - port %d\n", __func__, port->number);*/
	intfdata = port->serial->private;
	...
	if (status) {
		pr_debug( "%s - nonzero write bulk status " /*dev_dbg(&port->dev, "%s - nonzero write bulk status "*/
		    "received: %d\n", __func__, status);
...
/*	usb_serial_debug_data(debug, &port->dev, __func__, writesize, buffer);*/
	...
	if (retval) {
		...
		pr_err("%s - usb_submit_urb(write bulk) failed "
			"with status = %d\n", __func__, retval);
...
static void sierra_indat_callback(struct urb *urb)
{
	...
	if (status) {
		pr_debug( "%s: nonzero status: %d on" /*dev_dbg(&port->dev, "%s: nonzero status: %d on"*/
...
/*				usb_serial_debug_data(debug, &port->dev,
			...
			pr_debug( "%s: empty read urb" /*dev_dbg(&port->dev, "%s: empty read urb"*/
				" received\n", __func__);
		}
	}
	...
	if (port->port.count &&
		...
		if (err && err != -ENODEV)
			pr_err("resubmit read urb failed." "(%d)\n", err);
...
/*			usb_serial_debug_data(1, &port->dev, __func__,
	...
	if (port->port.count &&
		...
		err = usb_submit_urb(urb, GFP_ATOMIC);
		if (err && err != -ENODEV)
			pr_err("%s: resubmit intr urb " "failed. (%d)\n", __func__, err);
					...
					usb_complete_t callback)
{
	...
	if (urb == NULL) {
		pr_debug( "%s: alloc for endpoint %d failed\n", /*dev_dbg(&serial->dev->dev, "%s: alloc for endpoint %d failed\n",*/
			__func__, endpoint);
		return NULL;
	}
...
static int sierra_startup(struct usb_serial *serial)
{
	...
	if (serial->num_ports) {
		...
		if (!portdata) {
			pr_debug( "%s: No memory!\n", __func__); /*dev_dbg(&serial->dev->dev, "%s: No memory!\n", __func__);*/
			return -ENOMEM;
		}
	}
	...
	if (result == sizeof(fw_attr) && (fw_attr & SWI_FW_ATTR_PM_MASK) ) {
		pr_info("APM supported, enabling autosuspend.\n");
