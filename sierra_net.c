...
#else
#define pr_debug(fmt, ...) \
	no-printk(KERN_DEBUG pr_fmt(fmt), ##__VA_ARGS__)
#endif
...
static int sierra_net_send_sync(struct usbnet *dev)
{
	...
	pr_debug( "%s", __func__); /*dev_dbg(&dev->udev->dev, "%s", __func__);*/
...
{
	pr_debug( "%s %d", __func__, ctx_ix); /*dev_dbg(&(priv->usbnet->udev->dev), "%s %d", __func__, ctx_ix);*/
...
static int sierra_net_parse_lsi(struct usbnet *dev, char *data, int datalen)
{
	...
	if (datalen < sizeof(struct lsi_umts)) {
		deverr(dev, "%s: Data length %d, exp %Zu\n",
				__func__, datalen,
	...
	if (lsi->length != cpu_to_be16(SIERRA_NET_LSI_UMTS_STATUS_LEN)) {
		deverr(dev, "%s: LSI_UMTS_STATUS_LEN %d, exp %u\n",
	...
	if (lsi->protocol != SIERRA_NET_PROTOCOL_UMTS) {
		deverr(dev, "Protocol unsupported, 0x%02x\n",
			lsi->protocol);
		return -1;
	}
	...
	if ((lsi->link_type != SIERRA_NET_AS_LINK_TYPE_IPv4) &&
		...
		deverr(dev, "Link unavailable: 0x%02x",
			lsi->link_type);
		return 0;
	}
	...
	if (lsi->coverage == SIERRA_NET_COVERAGE_NONE
	   || lsi->coverage == SIERRA_NET_COVERAGE_NOPACKET) {
		deverr(dev, "No coverage, 0x%02x\n", lsi->coverage);
		return 0;
	}

	/* Validate the session state */
	if (lsi->session_state == SIERRA_NET_SESSION_IDLE) {
		deverr(dev, "Session idle, 0x%02x\n",
			lsi->session_state);
		return 0;
	}
...
static void sierra_net_kevent(struct work_struct *work)
{
	...
	if (test_bit(SIERRA_NET_EVENT_RESP_AVAIL, &priv->kevent_flags)) {
		...
		if (unlikely(len < 0)) {
			...
			pr_debug( "%s: Received status message," /*dev_dbg(&dev->udev->dev, "%s: Received status message,"*/
				" %04x bytes", __func__, len);
			sierra_net_printk_buf(buf, len);

			err = parse_hip(buf, len, &hh);
			if (err) {
				deverr(dev, "%s: Bad packet,"
					" parse result %d\n", __func__, err);
				kfree(buf);
				return;
			}
			...
			if (len != hh.hdrlen + hh.payload_len.word) {
				deverr(dev, "%s: Bad packet, received"
					" %d, expected %d\n",	__func__, len,
			...
			case SIERRA_NET_HIP_LSI_UMTSID:
				pr_debug( "LSI for ctx:%d", /*dev_dbg(&dev->udev->dev, "LSI for ctx:%d",*/
					hh.msgspecific.byte);
				sierra_net_handle_lsi(dev, buf, &hh);
				break;
			case SIERRA_NET_HIP_RESTART_ID:
				pr_debug( "Restart reported: %d," /*dev_dbg(&dev->udev->dev, "Restart reported: %d,"*/
						" stopping sync timer",
						hh.msgspecific.byte);
			...
			case SIERRA_NET_HIP_HSYNC_ID:
				pr_debug( "SYNC received"); /*dev_dbg(&dev->udev->dev, "SYNC received");*/
	...
	if (test_bit(SIERRA_NET_TIMER_EXPIRY, &priv->kevent_flags)) {
		...
		pr_debug( "Deferred sync timer expiry"); /*dev_dbg(&dev->udev->dev, "Deferred sync timer expiry");*/
		sierra_net_dosync(priv->usbnet);
	}

	if (priv->kevent_flags)
		pr_debug( "sierra_net_kevent done, " /*dev_dbg(&dev->udev->dev, "sierra_net_kevent done, "*/
...
static void sierra_net_defer_kevent(struct usbnet *dev, int work)
{
	...
	if (!schedule_work(&priv->sierra_net_kevent))
		pr_debug( "sierra_net_kevent %d may have been dropped", work); /*dev_dbg(&dev->udev->dev, "sierra_net_kevent %d may have been dropped", work);*/
	else
		pr_debug( "sierra_net_kevent %d scheduled", work); /*dev_dbg(&dev->udev->dev, "sierra_net_kevent %d scheduled", work);*/
}
...
static int sierra_net_bind(struct usbnet *dev, struct usb_interface *intf)
{
	...
	if (!is_whitelisted(ifacenum, &data->whitelist)) {
		pr_debug( "Ignoring interface: %d", ifacenum); /*dev_dbg(&dev->udev->dev, "Ignoring interface: %d", ifacenum);*/
		return -ENODEV;
	}
	numendpoints = intf->cur_altsetting->desc.bNumEndpoints;
	/* We have three endpoints, bulk in and out, and a status */
	if (numendpoints != 3) {
		pr_err("Expected 3 endpoints, found: %d",
			numendpoints);
	...
	if (status < 0) {
		pr_err("Error in usbnet_get_endpoints (%d)",
			status);
 ...
 */ 
		...
		pr_debug( "Enabling APM"); /*dev_dbg(&dev->udev->dev, "Enabling APM");*/
		usb_autopm_enable(dev->intf);
#endif
	}
	else {
		pr_info("Disabling APM - not supported");
	...
	if (!(status == sizeof(fwattr) && (fwattr & SWI_GET_FW_ATTR_MASK))) {
		...
		pr_err("Incompatible driver and firmware"
			" versions\n");
...
static int sierra_net_rx_fixup(struct usbnet *dev, struct sk_buff *skb)
{
	...
	while (likely(skb->len)) {
		...
		if (err) {
			if (netif_msg_rx_err(dev))
				deverr(dev, "Invalid HIP header %d\n",
					err);
		...
		if (!hh.extmsgid.is_present
			...
			if (netif_msg_rx_err(dev))
				deverr(dev, "HIP/ETH: Invalid pkt\n");
